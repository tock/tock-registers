    ...
    [$($name:ident)*] => {$(
        ...
        impl Address for $name {
            unsafe fn byte_add(self, offset: usize) -> Self {
                ...
                // We know that:
                //
                // 1. self.0 is not null, because it is a NonNull.
                // 2. `offset` remains within this register span and does not
                //    wrap, because of Address::byte_add's safety invariant.
                //
                // Adding a nonnegative offset to a positive address without
                // wrapping is guaranteed to produce a positive result, so
                // new_ptr is not null.
